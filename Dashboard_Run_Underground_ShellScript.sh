...
setup_azure_vm() {
    ...
    sudo apt-get install -y python3 python3-pip tshark scapy flask pandas scikit-learn matplotlib hping3 >> $LOGFILE 2>&1
...
# Function to clean up generated files after testing 
cleanup_files() {
   log_message "Cleaning up generated files..."
   rm -f traffic_capture.pcap features.csv normal_ping_traffic.csv malicious_ping_flood_traffic.csv >> $LOGFILE 2>&1 
...
main() {
   ...
   # Setup Dashboard for Visualization 
   setup_dashboard
